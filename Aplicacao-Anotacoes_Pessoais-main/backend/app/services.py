...
class UserService:
    ...
    def create_user(self, user_input):
        hashed = self.get_password_hash(user_input.password)
    ...
    def authenticate_user(self, user_input) -> User:
        with Session() as session:
            user = session.query(User).filter(User.username == user_input.username).first()
            if user and self.verify_password(user_input.password, user.password):
                return user
            return None
        
        
class NotesService:
    def create_notes(self, notes_input):
        with Session() as session:
    ...
    def update_notes(self, notes_input):
        with Session() as session:
